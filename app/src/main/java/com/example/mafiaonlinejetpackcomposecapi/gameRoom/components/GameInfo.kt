...
private fun PhaseCard(
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            InfoText("Mafia quantity: $mafiaCount")
            InfoText("Civilian quantity: $civilianCount")
            InfoText("Total alive: $totalAlive")

            Spacer(modifier = Modifier.height(10.dp))

            Text(
                text = "Current phase: ${if(phase.joinToString("") == "None") "..." else phase.slice(1..phase.lastIndex).joinToString(" ")}",
            ...
            Text(
                text = "Until next phase: ${if(counter == -1) "..." else counter} sec",
                color = Color(0xFFFFD700).copy(alpha = if (shouldBlink) alpha else 1f),
                fontSize = 14.sp
            )
