...
import com.leafcellteam.mafia.resources.characterLimitReached
import com.leafcellteam.mafia.resources.charactersLeft
...
import com.leafcellteam.mafia.resources.cooldownMessage
    ...
    BottomAppBar(
        ...
        Column(modifier = Modifier.fillMaxWidth()) {
            if (showCharacterCount) {
                Text(
                    text = when {
                        remainingChars <= 0 -> stringResource(Res.string.characterLimitReached)
                        remainingChars <= 20 -> "${stringResource(Res.string.charactersLeft)}: $remainingChars"
                        else -> "${stringResource(Res.string.charactersLeft)}: $remainingChars"
                    },
                    color = when {
            ...
            TextField(
                ...
                supportingText = if (!canSend) {
                    {
                        Text(
                            text = stringResource(Res.string.cooldownMessage),
                            fontSize = 11.sp,
